...
The same `core-image-minimal` for qemux86-64 as `yocto-6.0.2-live`, from the
same Yocto 6.0.2 tree on this machine, rebuilt with the ipk packaging backend:

    PACKAGE_CLASSES = "package_ipk"
    bitbake core-image-minimal

Every other fixture in this repository comes from an rpm-backend build, which
left the single most consequential transformation in the tool -- stripping the
...
* the architecture column is spelled `x86-64-v3` with hyphens, against
  ...
  where rpm writes `noarch`.

Paired with `yocto-6.0.2-live` it is the fixture that proves the point rather
than asserting it: 37 of the two builds' purls are byte-identical. The one
difference, `util-linux-flock`, is a real difference in image content -- the
ipk image installs 38 packages and the rpm image 37 -- not a parsing artifact.
